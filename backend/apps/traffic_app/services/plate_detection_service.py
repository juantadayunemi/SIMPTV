...
class PlateDetectionService:
    ...
    def __init__(self):
        ...
        self.cascade_path = os.path.join(
            settings.BASE_DIR, "models", "haarcascade_russian_plate_number.xml"
        )
        ...
        self._roboflow_model_id = getattr(
            settings, "ROBOFLOW_PLATE_MODEL", "license-plate-recognition-rxg4e/4"
        )
    ...
    def detect_plate_region(self, frame):
        ...
        if self._roboflow_api_key:
            try:
                ...
                all_plates.extend(
                    [(x, y, w, h, "roboflow") for x, y, w, h in roboflow_plates]
                )
                ...
                if len(roboflow_plates) > 0:
                    logger.info(
                        f"🎯 {len(roboflow_plates)} plate(s) detected (roboflow)"
                    )
        ...
        try:
            ...
            all_plates.extend(
                [(x, y, w, h, "haarcascade") for x, y, w, h in haar_plates]
            )
        ...
        try:
            ...
            all_plates.extend(
                [(x, y, w, h, "contours") for x, y, w, h in contour_plates]
            )
        ...
        if len(unique_plates) > 0:
            ...
            logger.info(
                f"🎯 {len(unique_plates)} plate(s) detected (FALLBACK: {' + '.join(methods)})"
            )
    ...
    def _detect_with_roboflow(self, frame):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"⚠️ Roboflow API error: {response.status_code} - {response.text[:200]}"
                )
                return []
            ...
            for pred in predictions:
                ...
                if confidence > 0.3:
                    ...
                    logger.debug(
                        f"  → Roboflow: ({x},{y},{w}x{h}) conf={confidence:.2f}"
                    )
    ...
    def read_plate_text(self, plate_image):
        ...
        try:
                    ...
                    if results:
                        for bbox, text, confidence in results:
                            ...
                            if cleaned_text:
                                ...
                                logger.debug(
                                    f"  {method_name}: '{cleaned_text}' (conf: {confidence:.2f})"
                                )
            ...
            valid_results = [
                r for r in all_results if self._validate_plate_text(r["text"])
            ]
            ...
            logger.info(
                f"✅ Mejor resultado: '{best['text']}' (conf: {best['confidence']:.2f}, método: {best['method']})"
            )
    ...
    def process_vehicle_detection(
        self, frame, vehicle_id, vehicle_type, video_name, analysis_id
    ):
        """
        ...
        NOTA: video_name ya no se usa en carpetas (solo analysis_id)
        """
        if not self.enabled:
            return None
        ...
        try:
            ...
            # Crear directorios usando SOLO analysis_id (evita problemas con nombres largos/especiales)
            roi_yolo_dir = os.path.join(
                settings.MEDIA_ROOT, "ROI YOLO", f"analysis_{analysis_id_str}"
            )

            placas_dir = os.path.join(
                settings.MEDIA_ROOT, "Placas", f"analysis_{analysis_id_str}"
            )
            ...
            if not plate_candidates or len(plate_candidates) == 0:
                logger.warning(
                    f"⚠️ No se detectaron candidatos en vehículo {vehicle_id}"
                )
            ...
            logger.info(
                f"🔍 Encontrados {len(plate_candidates)} candidatos, validando con OCR..."
            )
            ...
            for idx, (x, y, w, h) in enumerate(plate_candidates):
                ...
                if std_dev < 15:
                    logger.debug(
                        f"  ✗ Candidato {idx+1}: Descartado por bajo contraste (std={std_dev:.2f})"
                    )
                    continue
                ...
                try:
                    ...
                    if self._validate_plate_text(text):
                        logger.info(
                            f"  ✅ Candidato {idx+1}: Válido - '{text}' (conf: {conf:.2f})"
                        )
                        valid_plates.append(
                            {
                                "bbox": (x, y, w, h),
                        ...
                        logger.debug(
                            f"  ✗ Candidato {idx+1}: Texto no válido - '{text}'"
                        )
            ...
            if len(valid_plates) == 0:
                logger.warning(
                    f"⚠️ Ningún candidato pasó validación OCR para vehículo {vehicle_id}"
                )
            ...
            logger.info(
                f"🎯 Mejor placa: '{plate_text}' (conf: {confidence:.2f}, bbox: ({x},{y},{w}x{h}))"
            )
            ...
            logger.error(
                f"❌ Error procesando vehículo {vehicle_id}: {e}", exc_info=True
            )
            return None

    def _save_detection_to_json(
        self,
        video_name,
        vehicle_id,
        vehicle_type,
        ...
        NOTA: video_name se mantiene en JSON para referencia, pero NO en nombres de archivo
        """
        try:
            datos_folder = os.path.join(settings.MEDIA_ROOT, "datos")
            Path(datos_folder).mkdir(parents=True, exist_ok=True)

            # ✅ JSON único por análisis (solo usa analysis_id en nombre de archivo)
            json_filename = f"detections_analysis_{analysis_id}.json"
