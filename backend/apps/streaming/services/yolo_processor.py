...
class YOLOProcessor:
    ...
    def __init__(
        ...
        logger.info(
            "- ID Recovery: ENABLED (re-identifies lost tracks within 30 frames)"
        )
        ...
        logger.info(
            f"ROI enabled: {self.roi_enabled} (y >= {self.roi_y_min * 100:.0f}%)"
        )
    ...
    def es_carro_valido(self, box, frame_height, class_name="car"):
        ...
        if is_frontal:
            ...
            if area < min_area_frontal:
                logger.debug(
                    f"Rejected - small frontal area: {area:.0f} < {min_area_frontal}"
                )
                return False
            ...
            if ratio > 0.9:  # If ratio w/h > 0.9, it's too square/wide for frontal
                logger.debug(
                    f"Rejected - frontal not vertical enough: ratio={ratio:.2f} > 0.9"
                )
                return False
            ...
            if w < min_width_frontal:
                logger.debug(
                    f"Rejected - frontal too thin: width={w:.0f} < {min_width_frontal}"
                )
                return False
            ...
            logger.debug(
                f"✅ Frontal detection: {class_name} @ ({int(x1)},{int(y1)}) size=({int(w)}x{int(h)}), area={int(area)}px²"
            )
            return True
        ...
        # LATERAL VIEW DETECTION (strict filters as before)
            ...
            if ratio < 1.2 or ratio > 3.2:
                logger.debug(
                    f"Rejected - invalid lateral ratio: {ratio:.2f} outside 1.2-3.2"
                )
                return False
            ...
            if 0.85 < ratio < 1.25 and area < 12000:
                logger.debug(
                    f"Rejected - square object: ratio={ratio:.2f}, area={area:.0f}"
                )
                return False
            ...
            if h > w * 1.5:  # Increased from 1.3 to 1.5 for tolerance
                logger.debug(
                    f"Rejected - excessive height: h={h:.0f} > w*1.5={w*1.5:.0f}"
                )
                return False
            ...
            logger.debug(
                f"✅ Lateral detection: {class_name} @ ({int(x1)},{int(y1)}) size=({int(w)}x{int(h)}), area={int(area)}px²"
            )
            return True
    ...
    def process_frame(self, frame: np.ndarray) -> tuple[np.ndarray, list]:
        ...
        for r in results:
            for box in r.boxes:
                ...
                logger.debug(
                    f"Raw detection: {label} conf={conf:.2f} bbox=[{int(x1)},{int(y1)},{int(x2)},{int(y2)}]"
                )
                ...
                norfair_detection = Detection(
                    points=np.array([[centroid_x, centroid_y]]), scores=np.array([conf])
                )
        ...
        logger.info(
            f"YOLO detections: {len(norfair_detections)} -> Tracked objects: {len(tracked_objects)}"
        )
        ...
        for idx, track in enumerate(tracked_objects):
            ...
            if idx < len(detection_metadata):
                ...
                logger.debug(
                    f"Track {track_id} without current detection - skipping draw"
                )
                continue
        ...
        if len(detections) > 0:
            logger.info(
                f"Tracked: {len(detections)} objects | Unique total: {len(self.unique_vehicles)}"
            )
    ...
    def _calculate_matching_score(
        self, bbox1: tuple, bbox2: tuple, frames_lost: int
    ) -> tuple:
        """
    ...
    def _recover_lost_ids(self, detections: list) -> list:
        ...
        for lost_id in list(self.lost_tracks_history.keys()):
            if lost_id in current_ids:
                ...
                if (
                    self.lost_tracks_history[lost_id]["frames_lost"]
                    > self.max_lost_frames
                ):
                    logger.debug(
                        f"ID {lost_id} expired after {self.max_lost_frames} frames"
                    )
                    del self.lost_tracks_history[lost_id]
            ...
            if track_id not in self.tracked_bboxes:
                ...
                for lost_id, lost_data in self.lost_tracks_history.items():
                    ...
                    score, details = self._calculate_matching_score(
                        bbox_xyxy, last_bbox, frames_lost
                    )
        ...
        for disappeared_id in disappeared_ids:
            if disappeared_id not in self.lost_tracks_history:
                ...
                self.lost_tracks_history[disappeared_id] = {
                    "last_bbox": (
                        bbox_data["bbox"] if isinstance(bbox_data, dict) else bbox_data
                    ),
                    "frames_lost": 1,
                }
    ...
    def add_detection_to_session(self, frame: np.ndarray, detection: dict):
        ...
        logger.info(
            f"📊 Unique vehicles saved in session: {len(self.session_detections)}"
        )
