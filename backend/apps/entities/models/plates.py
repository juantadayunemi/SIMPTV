...
class DetectedPlateImageEntity(BaseModel):
    ...
    detectedPlateId = models.ForeignKey(
        "plates_app.DetectedPlate",
        on_delete=models.CASCADE,
...
class VehicleComplaintEntity(BaseModel):
    ...
    detectionId = models.ForeignKey(
        "plates_app.VehicleComplaintDetection",
        on_delete=models.CASCADE,
...
class ComplaintEvidenceImageEntity(BaseModel):
    ...
    complaintDetectionId = models.ForeignKey(
        "plates_app.VehicleComplaintDetection",
        on_delete=models.CASCADE,
        related_name="complaintevidenceimageentity_complaintdetection_set",
    )
    detectedPlateImageId = models.ForeignKey(
        "plates_app.DetectedPlateImage",
        on_delete=models.CASCADE,
...
class TemplateVariable(BaseModel):
    ...
    type = models.JSONField(
        default=dict, help_text="Reference to DataTypeKey interface"
    )
...
class PlateAlertFullReportDTO(BaseModel):
    ...
    alert = models.JSONField(
        default=dict, help_text="Reference to PlateAlertResponseDTO interface"
    )
...
class ProcessPlateDetectionRequestDTO(BaseModel):
    ...
    country = models.CharField(max_length=255)
    confidence = models.IntegerField()
    validateAgainstOfficialDB = models.BooleanField(
