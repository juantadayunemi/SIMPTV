...
class RealtimeDetectionEvent(BaseModel):
    """Abstract DLL model from TypeScript interface RealtimeDetectionEvent"""
    """USAGE: Inherit in other apps - class User(RealtimeDetectionEvent): pass"""
    ...
    trackId = models.UUIDField(default=uuid.uuid4, editable=False)
    ...
    class Meta:
        abstract = True  # DLL model - inherit in other apps
        verbose_name = "Abstract RealtimeDetectionEvent"
        verbose_name_plural = "Abstract RealtimeDetectionEvents"

    def __str__(self):
        return f'RealtimeDetectionEvent ({self.pk})'

class VideoAnalysisProgress(BaseModel):
    """Abstract DLL model from TypeScript interface VideoAnalysisProgress"""
    """USAGE: Inherit in other apps - class User(VideoAnalysisProgress): pass"""

    analysisId = models.IntegerField()
    currentFrame = models.IntegerField()
    totalFrames = models.IntegerField()
    progress = models.IntegerField()
    vehiclesDetected = models.IntegerField()
    platesDetected = models.IntegerField()
    currentTimestamp = models.IntegerField()
    ...
    class Meta:
        abstract = True  # DLL model - inherit in other apps
        verbose_name = "Abstract VideoAnalysisProgress"
        verbose_name_plural = "Abstract VideoAnalysisProgresss"

    def __str__(self):
        return f'VideoAnalysisProgress ({self.pk})'
