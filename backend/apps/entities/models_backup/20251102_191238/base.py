...
class BaseModelString(BaseModelDefault):
    """
    Base abstract model para entidades con ID STRING (VARCHAR - CUID autogenerado)

    Uso: Entidades con id: string en TypeScript
    Ejemplo: PredictionModelEntity, TrafficPredictionEntity, BatchPredictionEntity

    El ID es VARCHAR(50) con valores CUID generados automáticamente en la aplicación.

    Opciones de generación de ID:
    1. CUID (Conflict-free Unique ID) - Recomendado para este proyecto
       - Más pequeño que UUID (25 caracteres vs 36)
       - Mejor indexing en base de datos
       - Collision resistant

    2. UUID v4 - Alternativa universal
       - Estándar en muchas aplicaciones
       - 36 caracteres (más grande)
    ...
    id = models.CharField(
        ...
        db_column="id",
        verbose_name="ID",
        default=generate_string_id,  # Se autogenera con UUID v4 en Python
    )
