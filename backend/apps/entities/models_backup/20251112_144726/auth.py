...
class UserEntity(BaseModel):
    ...
    failedLoginAttempts = models.IntegerField(default=0, blank=True, null=True)
...
class UserRoleEntity(BaseModel):
    ...
    user = models.ForeignKey('auth_app.User', on_delete=models.CASCADE, related_name='userroleentity_user_set')
...
class RolePermissionEntity(BaseModel):
    """Abstract DLL model from TypeScript interface RolePermissionEntity"""
    """USAGE: Inherit in other apps - class User(RolePermissionEntity): pass"""

    role = models.CharField(max_length=50, choices=USER_ROLES_CHOICES)
    permission = models.CharField(max_length=100)
    isGranted = models.BooleanField(default=True)
    grantedBy = models.CharField(max_length=255, blank=True, null=True)
    grantedAt = models.DateTimeField()
    expiresAt = models.DateTimeField(blank=True, null=True)

    class Meta:
        abstract = True  # DLL model - inherit in other apps
        verbose_name = "Abstract RolePermissionEntity"
        verbose_name_plural = "Abstract RolePermissionEntitys"

    def __str__(self):
        return f'RolePermissionEntity ({self.pk})'

class UserPermissionOverrideEntity(BaseModel):
    """Abstract DLL model from TypeScript interface UserPermissionOverrideEntity"""
    """USAGE: Inherit in other apps - class User(UserPermissionOverrideEntity): pass"""

    user = models.ForeignKey('auth_app.User', on_delete=models.CASCADE, related_name='userpermissionoverrideentity_user_set')
    permission = models.CharField(max_length=100)
    isGranted = models.BooleanField(default=False)
    overrideReason = models.TextField(blank=True, null=True)
    grantedBy = models.CharField(max_length=255)
    grantedAt = models.DateTimeField()
    expiresAt = models.DateTimeField(blank=True, null=True)

    class Meta:
        abstract = True  # DLL model - inherit in other apps
        verbose_name = "Abstract UserPermissionOverrideEntity"
        verbose_name_plural = "Abstract UserPermissionOverrideEntitys"

    def __str__(self):
        return f'UserPermissionOverrideEntity ({self.pk})'

class CustomerEntity(BaseModelString):
...
class UserInfoDTO(BaseModelString):
