...
export const Sidebar: React.FC = () => {
  ...
  return (
    <div className={`bg-primary-600 text-white min-h-screen flex flex-col transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'} shadow-xl`}>
      {/* Logo */}
        ...
        <div className="flex items-center justify-between">
          ...
          {isCollapsed && (
            <button 
              ...
              title="Expandir menú"
            >
              {/* Icono de tráfico cuando está colapsado - clickeable para expandir */}
              <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center shadow-lg hover:shadow-xl transition-shadow">
                <svg className="w-6 h-6 text-blue-600" fill="currentColor" viewBox="0 0 24 24">
                  {/* Semáforo */}
                  <rect x="9" y="2" width="6" height="18" rx="3" fill="currentColor" opacity="0.3"/>
                  <circle cx="12" cy="6" r="2" fill="#ef4444"/>
                  <circle cx="12" cy="12" r="2" fill="#eab308"/>
                  <circle cx="12" cy="18" r="2" fill="#22c55e"/>
                  {/* Cámara de seguridad */}
                  <path d="M16 8h3a1 1 0 011 1v2a1 1 0 01-1 1h-3l-1-2 1-2z" fill="currentColor"/>
                </svg>
              </div>
            </button>
          )}
      ...
      {!isCollapsed && (
        <div className="p-4 border-t border-gray-700/50 bg-gray-800/50">
          ...
          <button
            ...
            className="w-full flex items-center px-3 py-2 text-sm text-gray-300 hover:text-white hover:bg-red-600/20 rounded-lg transition-colors group"
          >
      ...
      {isCollapsed && (
        <div className="p-3 border-t border-gray-700/50 space-y-2">
          ...
          <button
            onClick={handleLogout}
            className="w-10 h-10 mx-auto flex items-center justify-center text-gray-400 hover:text-red-400 hover:bg-red-600/20 rounded-lg transition-colors group"
            title="Cerrar Sesión"
          >
          ...
          {/* Botón de expansión adicional */}
          <button
            onClick={() => setIsCollapsed(false)}
            className="w-10 h-10 mx-auto flex items-center justify-center text-gray-400 hover:text-blue-400 hover:bg-blue-600/20 rounded-lg transition-colors group mt-2"
            title="Expandir menú"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
